...
#    cp /opt/out/instance/server/default/conf/data-default.zip /opt/out/instance/server/default/data/drop-in-deployer/data.zip
    ...
    curl -X PUT --header 'Content-Type: application/json' --header 'X-XSRF-Header: PingFederate' --data '@licenseagree.json' https://localhost:9999/pf-admin-api/v1/license/agreement --insecure
    curl -X POST --header 'Content-Type: application/json' --header 'X-XSRF-Header: PingFederate' --data '@createadmin.json' https://localhost:9999/pf-admin-api/v1/administrativeAccounts --insecure
